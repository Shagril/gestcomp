...
echo'option.text = "'.$donnees['codeactivite'].'";
	...
	option = document.createElement("option");;';	echo('	document.getElementById("reformuler").innerHTML += "<div id=\"reformuler-"+choixa+"\">'.$donnees['codeactivite'].'<br/></div>";');
...
function Supprimer()
{
	...
	while (i<=insert.length-1 )
	{
		...
		{
			value = insert.options[i].value;			document.getElementById('reformuler-'+value).outerHTML = null;
...
<body>
	<form name="activite">
		...
		<select name="select" size="10" STYLE="width:900" onchange="modif();" disabled>
...
<?php
	...
	$req1="select codeactivite from mettre_en_oeuvre where codesituation=".$situation;//modif
...
?>
	...
	<div  >
		<select name="insert" id="insert" size="10" STYLE="width:900" disabled>
		<?php
	$req="SELECT codeactivite FROM mettre_en_oeuvre where codesituation=".$situation." order by codeactivite";
	$reponse = $db->query($req);
	$processus = "";
	while($donnees = $reponse->fetch())
	{
