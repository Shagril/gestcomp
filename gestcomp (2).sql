...
--
-- Client :  127.0.0.1
-- Généré le :  Mar 03 Mai 2016 à 20:59
...
CREATE TABLE IF NOT EXISTS `commenter` (
  ...
  `codeSituation` int(11) DEFAULT NULL,
  `codeutilisateur` varchar(32) NOT NULL,
...
INSERT INTO `commenter` (`dateHeure`, `codeSituation`, `codeutilisateur`, `commentaire`) VALUES
('2016-05-03 20:56:09', 46, 'prof@local.fr', 'ceci est un commentaire');
...
INSERT INTO `etudiant` (`MAILLOGIN`, `CODEPROMOTION`, `numeroINE`) VALUES
('achassat@local.fr', '6', 1111),
('eleve@local.fr', '6', 2222),
('fcadet@local.fr', '7', 3333);
...
INSERT INTO `localisation` (`CODELOC`, `libelle`) VALUES
('1', 'Organisation'),
...
--
-- Contenu de la table `mettre_en_oeuvre`
--

INSERT INTO `mettre_en_oeuvre` (`CODESITUATION`, `CODEACTIVITE`, `REFORMULATION`) VALUES
('47', 'A3.3.5', NULL),
('47', 'A3.3.3', NULL),
('46', 'A1.1.3', ''),
('46', 'A1.1.2', ''),
('46', 'A1.1.1', 'reformulation');
...
CREATE TABLE IF NOT EXISTS `promotion` (
  `codepromotion` int(11) NOT NULL AUTO_INCREMENT,
  `CODEPARCOURS` char(32) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `situation` (
  `CODESITUATION` int(11) NOT NULL AUTO_INCREMENT,
...
INSERT INTO `situation` (`CODESITUATION`, `CODETYPESITUATION`, `CODESOURCE`, `CODECADRE`, `CODELOC`, `CODEUTILISATEUR`, `LIBELLE`, `DESCRIPTION`, `CONTEXTE`, `DATEDEBUT`, `DATEFIN`, `ENVIRONNEMENTTECHNOLOGIQUE`, `MOYENS`, `AVISPERSONNEL`) VALUES
(47, '1', '1', '1', '1', 'eleve@local.fr', 'situation 2', 'description situation 2', '', '2016-05-01', '2016-05-03', '', '', ''),
(46, '1', '1', '1', '1', 'eleve@local.fr', 'situation 1', 'description situation', 'contexte', '2016-05-02', '2016-05-03', '', '', '');

-- --------------------------------------------------------

--
-- Structure de la table `situationobligatoire`
--

CREATE TABLE IF NOT EXISTS `situationobligatoire` (
...
--
-- Contenu de la table `situationobligatoire`
--

INSERT INTO `situationobligatoire` (`CODEOBLIGATIONSITUATION`, `CODESITUATION`) VALUES
('1', '46'),
('4', '46');
...
--
-- Contenu de la table `situation_cadre`
--

INSERT INTO `situation_cadre` (`CODECADRE`, `LIBELLE`) VALUES
('1', 'Equipe'),
('2', 'Seul');
...
--
-- Contenu de la table `situation_obligatoire`
--

INSERT INTO `situation_obligatoire` (`CODEOBLIGATIONSITUATION`, `LIBELLE`) VALUES
('1', 'Production d’une solution logicielle et d’infrastructure'),
('2', 'Prise en charge d’incidents et de demandes d’assistance'),
('3', 'Élaboration de documents relatifs à la production et à la fourniture de services'),
('4', 'Mise en place d’un dispositif de veille technologique');
...
CREATE TABLE IF NOT EXISTS `situation_production` (
  `codeproduction` int(11) NOT NULL AUTO_INCREMENT,
  `CODESITUATION` char(32) NOT NULL,
...
--
-- Contenu de la table `situation_production`
--

INSERT INTO `situation_production` (`codeproduction`, `CODESITUATION`, `DESIGNATION`, `URL`) VALUES
(3, '46', 'lien n)1', 'lien.fr');
...
INSERT INTO `source` (`CODESOURCE`, `LIBELLE`) VALUES
('1', 'Stage'),
('2', 'TP'),
('3', 'PPE');
...
--
-- Contenu de la table `type_situation`
--

INSERT INTO `type_situation` (`CODETYPESITUATION`, `LIBELLE`) VALUES
('1', 'Vécu'),
('2', 'Observé'),
('3', 'Simulé');
...
('fcadet@local.fr', 'cadet', 'flavien', 'acdc42d691e7f65c6b0aa443c5b48ea2e9e45872', '', 0),
('achassat@local.fr', 'chassat', 'alexis', 'acdc42d691e7f65c6b0aa443c5b48ea2e9e45872', '', 0),
('prof@local.fr', 'profNom', 'profPrenom', 'd9f02d46be016f1b301f7c02a4b9c4ebe0dde7ef', '', 0),
('eleve@local.fr', 'eleveNom', 'elevePrenom', '0e9a7fdc4821370a252df21582a4a656e81e0687', '', 0),
('admin@local.fr', 'admin', 'admin', 'd033e22ae348aeb5660fc2140aec35850c4da997', '', 0);

-- --------------------------------------------------------

--
-- Doublure de structure pour la vue `vue_utilisateur`
--
CREATE TABLE IF NOT EXISTS `vue_utilisateur` (
`mailLogin` varchar(100)
,`mdp` varchar(128)
,`nom` char(32)
,`prenom` char(32)
,`type` varchar(5)
...
--
-- Structure de la vue `vue_utilisateur`
--
DROP TABLE IF EXISTS `vue_utilisateur`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vue_utilisateur` AS select `u`.`MAILLOGIN` AS `mailLogin`,`u`.`MDP` AS `mdp`,`u`.`NOM` AS `nom`,`u`.`PRENOM` AS `prenom`,'Eleve' AS `type`,`u`.`AVATAR` AS `avatar` from (`utilisateur` `u` join `etudiant` `e` on(((`u`.`MAILLOGIN` = `e`.`MAILLOGIN`) and (`u`.`estSupprimer` = 0)))) union select `u`.`MAILLOGIN` AS `mailLogin`,`u`.`MDP` AS `mdp`,`u`.`NOM` AS `nom`,`u`.`PRENOM` AS `prenom`,'Prof' AS `type`,`u`.`AVATAR` AS `avatar` from (`utilisateur` `u` join `professeur` `p` on(((`u`.`MAILLOGIN` = `p`.`MAILLOGIN`) and (`u`.`estSupprimer` = 0) and (`p`.`ESTADMINISTRATEUR` = 0)))) union select `u`.`MAILLOGIN` AS `mailLogin`,`u`.`MDP` AS `mdp`,`u`.`NOM` AS `nom`,`u`.`PRENOM` AS `prenom`,'Admin' AS `type`,`u`.`AVATAR` AS `avatar` from (`utilisateur` `u` join `professeur` `p` on(((`u`.`MAILLOGIN` = `p`.`MAILLOGIN`) and (`u`.`estSupprimer` = 0) and (`p`.`ESTADMINISTRATEUR` <> 0))));
